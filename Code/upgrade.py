...
class Upgrade():
    def __init__(self,player):
        ...
        self.player = player
        self.attributes_num = len(player.stats) 
    ...
    def input(self):
        ...
        if self.can_move:
            if keys[pygame.K_d] and self.selection_index < self.attributes_num -1:
    ...
    def create_items (self):
        self.item_list = []
        for item, index in enumerate(range(self.attributes_num)):
            #horizontal position
            full_width = self.display_surface.get_size()[0]
            increment = full_width // self.attributes_num
            left = (item * increment) + (increment - self.width) // 2
            #vertical position
            top = self.display_surface.get_size()[1] * 0.1
            #create the object
            self.item_list. append (Item (left,top, self.width, self.height, index, self.font))
...
class Item:
    ...
    def display_name (self,surface ,name, cost, selected):
        ...
        #surface
        cost = int (cost)
        cost_surf = self.font.render (f"{cost}", False, color)
